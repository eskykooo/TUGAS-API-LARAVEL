    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            $catMeta = [
                ...
                'review' => ['icon'=>'fa-star','color'=>'bg-brutal-yellow'],
                'tips-tricks' => ['icon'=>'fa-lightbulb','color'=>'bg-brutal-green'],
                'game-guides' => ['icon'=>'fa-book','color'=>'bg-brutal-orange'],
                'hardware' => ['icon'=>'fa-microchip','color'=>'bg-brutal-red'],
                'gaming-culture' => ['icon'=>'fa-users','color'=>'bg-brutal-yellow'],
            ];
            @endphp
            @foreach($categories as $cat)
            @php $meta = $catMeta[$cat->slug] ?? ['icon'=>'fa-folder','color'=>'bg-dark-border']; @endphp
            <a href="/categories/{{ $cat->slug }}" data-aos="fade-up" data-aos-delay="{{ $loop->index * 80 }}" class="glass-card p-4 sm:p-6 flex items-center gap-3 sm:gap-4 group">
