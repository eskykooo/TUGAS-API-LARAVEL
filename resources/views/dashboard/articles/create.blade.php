...
.seo-bar-bg {
    background: linear-gradient(90deg, #FF1744, #FFD600, #00E676);
}
    ...
    <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="font-orbitron text-xl sm:text-2xl font-black text-white uppercase tracking-wide">Buat Artikel Baru</h1>
                    <p class="text-gray-500 text-xs font-bold uppercase tracking-wider mt-0.5">Tulis konten gaming berkualitas</p>
                </div>
            </div>
                ...
                <div class="lg:col-span-3 space-y-5">
                    ...
                    <div class="glass-panel border border-brutal-orange/20 hover:border-brutal-orange/40 transition-colors p-5 sm:p-6 rounded">
                        ...
                        <input type="text" name="title" value="{{ old('title') }}"
                               ...
                               placeholder="Cth: Review Black Myth Wukong — Layak Jadi Game of the Year?"
                               maxlength="200"
                               required autofocus>
                        @error('title')<p class="text-brutal-red text-xs font-bold mt-1.5">{{ $message }}</p>@enderror
                        <div class="flex items-center justify-between mt-2">
                            <p class="text-xs text-gray-500 font-bold" x-show="slug">
                                <span class="text-gray-600">Slug:</span>
                                <span x-text="slug" class="text-brutal-orange"></span>
                            </p>
                            <p class="text-xs text-gray-600 font-bold" x-show="!slug">Slug akan dibuat otomatis</p>
                        </div>
                    </div>
                    ...
                    <div class="glass-panel border border-brutal-orange/20 hover:border-brutal-orange/40 transition-colors p-5 sm:p-6 rounded">
                        ...
                        <textarea name="excerpt" rows="3"
                                  ...
                                  placeholder="Tulis ringkasan singkat — akan muncul di hasil pencarian &amp; sosial media..."
                                  maxlength="200">{{ old('excerpt') }}</textarea>
                    </div>
                    ...
                    <div x-show="showPreview" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0"
                        ...
                        <div class="border-l-4 border-brutal-orange pl-4">
                            <div class="flex items-center gap-2 text-[10px] text-gray-500 font-bold uppercase tracking-wider mb-2">
                                @if(old('category_id'))
                                <span class="px-2 py-0.5 border text-[10px] font-bold" style="border-color: #FF6B35; color: #FF6B35;">{{\App\Models\Category::find(old('category_id'))?->name ?? 'Kategori'}}</span>
                                @else
                                <span class="px-2 py-0.5 border border-gray-600 text-gray-600 text-[10px] font-bold uppercase">Kategori</span>
                                @endif
                                <span><i class="far fa-clock mr-1"></i> <span x-text="readingTime"></span> menit baca</span>
                            </div>
                            <h2 class="font-orbitron text-xl sm:text-2xl font-bold text-white leading-tight" x-html="title || 'Judul Artikel Akan Tampil di Sini'"></h2>
                        </div>
                ...
                <div class="lg:col-span-2 space-y-4">
                        ...
                        <div class="border-2 border-dashed border-dark-border hover:border-brutal-orange/60 transition-colors p-4 text-center cursor-pointer rounded relative group thumbnail-drop-zone"
                            ...
                            <template x-if="!thumbnailPreview">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 font-bold uppercase tracking-wider">Klik atau seret gambar</p>
                    ...
                    <div class="glass-panel border border-brutal-orange/20 hover:border-brutal-orange/40 transition-colors p-5 rounded">
                        ...
                        <div class="mt-3 pt-3 border-t border-dark-border">
                            <p class="text-[10px] text-gray-600 font-bold uppercase tracking-wider flex items-center gap-1.5">
                                ...
                                <span x-text="status === 'draft' ? 'Artikel akan disimpan sebagai draf.' : 'Artikel akan dikirim ke admin untuk ditinjau.'"></span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2.5 pt-1">
                        <button type="submit" @click="status = 'draft'"
                                class="w-full flex items-center justify-center gap-2.5 px-5 py-3.5 bg-dark-bg border-2 border-dark-border text-gray-300 font-bold text-sm uppercase tracking-wider hover:border-brutal-orange hover:text-brutal-orange hover:bg-dark-card transition-all duration-200 rounded">
                            <i class="fas fa-save"></i>
                            Simpan sebagai Draf
                        </button>
                        <button type="submit" @click="status = 'published'"
                                class="w-full flex items-center justify-center gap-2.5 px-5 py-3.5 bg-brutal-orange text-brutal-black font-black text-sm uppercase tracking-wider border-2 border-brutal-black hover:shadow-brutal-lg hover:-translate-y-0.5 active:translate-y-0.5 transition-all duration-200 rounded btn-pulse">
                    ...
                    <div class="flex items-center gap-2 py-2">
                        ...
                        <button type="button" @click="resetForm"
                                class="flex items-center gap-2 px-4 py-2 text-xs font-bold text-gray-500 uppercase tracking-wider hover:text-brutal-red transition-colors ml-auto">
