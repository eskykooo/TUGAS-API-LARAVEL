...
<section class="min-h-screen bg-dark-bg py-16 sm:py-20">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-3 mb-8">
            ...
            <div>
                <h1 class="font-orbitron text-2xl sm:text-3xl font-black text-white uppercase tracking-wide">Edit Artikel</h1>
                <p class="text-gray-500 text-sm font-bold uppercase tracking-wider">Perbarui konten artikel</p>
            </div>
        </div>

        <form method="POST" action="/dashboard/articles/{{ $article->id }}" enctype="multipart/form-data" class="space-y-6" x-data="{ title: '{{ str_replace("'", "\\'", $article->title) }}', content: '', status: '{{ $article->status }}', preview: false }">
            @csrf
            @method('PUT')
            ...
            <div class="bg-dark-card border-2 border-dark-border p-5 sm:p-6 space-y-5">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $article->title) }}" x-model="title" class="input-brutal text-lg font-bold @error('title') border-brutal-red @enderror" placeholder="Judul artikel..." required>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <textarea name="excerpt" rows="3" class="input-brutal text-sm" placeholder="Ringkasan singkat artikel...">{{ old('excerpt', $article->excerpt) }}</textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-400 mb-1.5 uppercase tracking-wider">Thumbnail</label>
                        @if($article->thumbnail_url)
                        <div class="mb-2">
                            <img src="{{ $article->thumbnail_url }}" class="w-full h-32 object-cover border-2 border-dark-border">
                        </div>
                        @endif
                        <div class="border-2 border-dashed border-dark-border p-4 sm:p-6 text-center hover:border-brutal-orange transition cursor-pointer" x-data="{ thumbnailName: '' }">
                            ...
                            <label for="thumbnailInput" class="cursor-pointer block">
                                ...
                                <p class="text-sm text-gray-500 font-bold uppercase tracking-wider" x-show="!thumbnailName">Klik untuk ganti</p>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <select name="category_id" class="select-brutal text-sm" required>
